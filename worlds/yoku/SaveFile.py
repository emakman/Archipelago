...
class SaveFile:
    ...
    def __init__(self, id: str, player: str, base: str, save_items: list[SaveItem]) -> None:
        self.id = id
        self.player = player
        self.base = base
    ...
    def save(self) -> None:
        with open(f"{self.base}.apyoku", 'w') as f:
                r = deepcopy(vh_save)
                r["_global"]["quests_completed"] += [f"\u001eamwr\u001e{self.id}\u001e{self.player}"]
                for id in order:
